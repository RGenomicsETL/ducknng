...
- Hardened the native `ducknng_quack_batch` decoder against malicious length arithmetic. Reader offset checks now use subtraction-form bounds checks, blob lengths that do not fit `size_t` are rejected before casting, fixed-width vector byte counts use checked multiplication, list/array child sizes and row counts are range-checked, cumulative row counts cannot wrap, and a single source chunk cannot exceed DuckDB's output vector capacity. Malformed Quack-derived bodies now fail with a codec error instead of risking heap out-of-bounds reads.
- Added deterministic property/fuzz regression fixtures for the two Quack arithmetic bugs: a fixed-width vector whose `width * rows` wrapped to the supplied blob length, and a variable-width string length crafted to wrap the reader offset back into earlier payload bytes. The existing randomized property tests continue to fuzz wire, transport, size arithmetic, and Quack skip paths with fixed seeds for reproducibility.
- Added SQL regression coverage for rejected CRLF header values, route header-builder validation, malformed Quack body parsing, profile auth-header collisions, and TLS client CA verification failures on both NNG TLS and HTTPS client paths. The self-published release workflow now also smoke-checks header control-character rejection, malformed Quack rejection, profile auth-header collisions, and TLS client CA verification on native loadable release artifacts.
