...
- Backported the checkpointed DuckDB workflow example from the development line. `examples/durable_workflows/` carries an Absurd-style task, lease, checkpoint, sleep, retry, and cached-event state machine whose transitions are single SQL statements, with Python and R workers. `make durable_workflow_smoke` and `make durable_workflow_r_smoke` run its checks.
- Backported SQL-defined RPC methods from the development line. `ducknng_register_sql_method(name, handler_sql, request_schema_json[, requires_auth])` registers a manifest-visible method in the `sql_method` family whose handler is server-owned SQL: the caller sends one JSON object, the statements run in one transaction on the service's request connection with the payload bound to their single parameter, and the first value of the last statement becomes the JSON reply. Any failure rolls back and returns an error frame. Admission is unchanged, so such an endpoint can sit behind mTLS, allowlists, and the SQL authorizer. `ducknng_request_subject()` gives handler SQL the verified caller. This branch has no general execution subject, so the accessor is populated only inside SQL method handlers, where the development line populates it for every RPC handler. Covered by `test/sql/ducknng_sql_methods.test`.
