...
| `unstable_new_vector_functions`                | `duckdb_create_selection_vector`, `duckdb_destroy_selection_vector`, `duckdb_selection_vector_get_data_ptr`, `duckdb_unsafe_vector_assign_string_element_len`, `duckdb_vector_copy_sel`                                                                                                                                                 |     5 |
...
selections and unsupported FSST vectors. Canonical chunks repeat and
validate the `DataChunk` logical-type list. The pure-C encoder remains
flat because DuckDB’s C API does not expose source compression state. A
normal SQL query cannot reliably force a particular compressed
source-vector shape through that API, so the executable
compressed-vector fixtures are generated by
