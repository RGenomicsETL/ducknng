...
`src/ducknng_duckdb_streaming_compat.c`, because DuckDB v1.5.0 does not
...
DuckDB v1.5.0 has `GEOMETRY` and `VARIANT`, but the current
...
`ducknng_enable_log_capture()` is unsafe to call under DuckDB v1.5.0 due
...
harnesses. The extension builds against DuckDB v1.5.0 using the C API
only.
