...
This is a rendered walkthrough of the subscriber gateway topology in `ducknng`. All three services run inside a single in-process DuckDB connection — no separate worker processes required — because each service uses `service_serialized_connection`, giving it a dedicated execution lane.
...
<details>
<summary><strong>Gateway Setup</strong></summary>

See the setup chunk above for the full service configuration, table definitions, token assignment, and health-check used to initialise this demo.

</details>

## Topology

One gateway service owns the public HTTP edge, identity resolution, and subscriber lookup. Two private backend services expose ordinary `ducknng` query-session services over TCP. All three share one in-memory DuckDB database, and all three use `service_serialized_connection` so they each have their own dedicated connection lane.
...
Because gateway route SQL calls `ducknng_open_query_raw(backend_url, ...)` synchronously, the gateway's connection blocks while waiting for the backend reply. Since each backend listens on its own connection, it can respond independently — no deadlock, even though all three services share one DuckDB runtime. In a production deployment each subscriber backend would be a separate DuckDB process with its own isolated database.
...
`gateway_identities` resolves bearer tokens into a tenant and principal. `gateway_subscribers` resolves that tenant to one private backend URL. Nothing in the public route SQL needs to know whether the backend plane has two workers or two hundred.

## Starting the services

All three services start in one DuckDB connection. Backend services start first so their assigned TCP ports are available when building the gateway's subscriber table.
...
The public edge is three HTTP routes, all registered with a 1 MiB body limit:
...
That keeps the route layer transport-local and gateway-owned. The public client sees HTTP plus Arrow batches; the private worker plane stays on the ordinary `ducknng` session contract.
...
The response body is an Arrow IPC batch. The gateway resolved the bearer token, chose `alice_worker` as the backend, and embedded backend session state in the opaque `X-Ducknng-Next-Token`.

```{r alice-start-body, echo = FALSE}
hex <- raw_hex(alice_start$data)
rows <- dbGetQuery(con, sprintf(
  "SELECT * FROM ducknng_parse_body(from_hex('%s'),
     'application/vnd.apache.arrow.stream') LIMIT 4",
  hex
))
kable(rows)
```
...
```{r alice-fetch-body, echo = FALSE}
hex <- raw_hex(alice_fetch$data)
rows <- dbGetQuery(con, sprintf(
  "SELECT * FROM ducknng_parse_body(from_hex('%s'),
     'application/vnd.apache.arrow.stream') LIMIT 4",
  hex
...
The public continuation token is gateway-owned. It carries tenant and subscriber affinity plus the private backend `session_id` and `session_token` without exposing them directly to the client.
...
```{r bob-start-body, echo = FALSE}
hex <- raw_hex(bob_start$data)
rows <- dbGetQuery(con, sprintf(
  "SELECT * FROM ducknng_parse_body(from_hex('%s'),
     'application/vnd.apache.arrow.stream') LIMIT 4",
  hex
...
The public start route did not change. Only the authenticated identity changed, and the gateway routed the request to `bob_worker` rather than `alice_worker`.
...
kable(data.frame(
  ...
  body   = rawToChar(body_raw(wrong_close$data)),
  stringsAsFactors = FALSE
))
```

The `403` is the important ownership boundary: a gateway token is not a cross-tenant escape hatch.
...
kable(data.frame(
  ...
  body   = rawToChar(body_raw(right_close$data)),
  stringsAsFactors = FALSE
))
```
...
The `service_serialized_connection` model makes all of this work within a single DuckDB runtime: each service has its own connection lane, so a gateway route handler can make synchronous NNG calls to sibling services without deadlocking.
...
try(dbExecute(con, "SELECT ducknng_stop_server('gateway')"),          silent = TRUE)
try(dbExecute(con, "SELECT ducknng_stop_server('subscriber_bob')"),   silent = TRUE)
try(dbExecute(con, "SELECT ducknng_stop_server('subscriber_alice')"), silent = TRUE)
