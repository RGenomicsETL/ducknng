...
- [~] Harden the session query family now that SQL helpers exist.
  - Current state:
    ...
    - frame-field scalar accessors now exist as `ducknng_frame_payload()`, `ducknng_frame_payload_text()`, and `ducknng_frame_error_text()`, which makes per-request route SQL practical without bind-time table decoding
