...
The dynamic row schema returned by `fetch` is fixed for the life of the session. `fetch` must not change column names or logical types across batches of the same session. End-of-stream, cancellation acknowledgement, and already-closed status are communicated through JSON control metadata, not through empty Arrow tables with sentinel columns. Fetch metadata such as `result_handle`, terminal `batch_index`, and echoed `correlation_id` is therefore part of the control contract today; if row-bearing metadata is ever needed later, it should be introduced as a generic negotiated extension rather than by pretending Arrow IPC has been replaced.
