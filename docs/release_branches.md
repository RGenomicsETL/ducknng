...
The `release/duckdb-1.5.0` branch carries the current ducknng implementation without feature substitutions: DuckDB 1.5.0 exposes every C API group used by this release. Its build, SQL suite, property sanitizers, and release workflow must all run against the exact 1.5.0 runtime.
...
```sql
LOAD '/path/to/ducknng-v0.1.2-duckdb1.5.2-linux_amd64.duckdb_extension';
```
