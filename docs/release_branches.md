...
The release workflow runs the HTTP smoke test and verifies the synchronous side-effecting transport/service scalars are `VOLATILE` on native platforms where the matching DuckDB Python runtime can load the built artifact directly: Linux and macOS. The Windows MinGW/Rtools artifact is smoke-loaded through the matching R `duckdb` package, which uses the Rtools/MinGW runtime and can load the `windows_amd64_mingw` extension. The `wasm_eh` artifact is a duckdb-wasm side module and is build-validated here; browser runtime proof remains covered by the wasm/browser workflow and tests rather than by native `LOAD`.
