...
For deeper write-ups see `docs/protocol.md`, `docs/manifest.md`, `docs/security.md`, `docs/registry.md`, `docs/transports.md`, `docs/http.md`, and related files under `docs/`. `NEWS.md` summarizes landed changes. TLS supports both file-backed and in-memory PEM material; `auth_mode = 2` enables required mTLS peer verification for `tls+tcp://`, `wss://`, and `https://` services.
...
`ducknng` encodes and decodes Arrow IPC payloads through hand-written type mappings in C using [nanoarrow](https://github.com/apache/arrow-nanoarrow). The extension targets the stable DuckDB C API only, which means type dispatch is explicit rather than relying on DuckDB's unstable built-in Arrow conversion paths. The trade-off is deliberate: the type mapping table in `docs/types.md` is maintained by hand, but the extension loads across DuckDB minor releases without recompilation.

The row path carries temporal, decimal, list, struct, union, large-string, large-binary, fixed-size-binary, and duration values.
...
By default these route handlers run on the backward-compatible `shared_serialized_connection` lane. Services can switch before traffic to `service_serialized_connection` or `request_connection` with `ducknng_set_service_execution_model(...)`. Those models use pre-opened DuckDB execution-pool connections from the same database handle; handlers should depend on catalog-visible objects rather than temp tables or temp macros from the init connection.

A public subscriber gateway is a good fit for this layer using separate backend DuckDB processes; the walkthrough lives in `docs/subscriber_gateway_demo.md`.
...
For mTLS, create a TLS handle with `auth_mode = 2`. The dispatcher derives the caller identity from the first verified peer certificate SAN as `tls:san:<value>`, falling back to the common name as `tls:cn:<common-name>`. Sessions opened over mTLS are bound to that verified peer identity. `ducknng_set_tls_peer_allowlist(...)` and `ducknng_set_service_peer_allowlist(...)` manage an exact identity allowlist. `ducknng_list_servers()` exposes `execution_model`, `active_pipes`, `tls_enabled`, `tls_auth_mode`, `peer_allowlist_active`, `ip_allowlist_active`, `sql_authorizer_active`, and related counters. Individual registry-backed methods can also require verified peer identity with `ducknng_set_method_auth(...)`.

Service resource limits run as fast C checks before SQL dispatch: `ducknng_set_service_limits(...)` caps open sessions, active pipes, inflight requests, and per-principal concurrency, reply-byte budgets, and session-open counts. For policy that depends on tables, tenants, method names, or deployment-specific rules, install a SQL authorizer with `ducknng_set_service_authorizer(name, authorizer_sql)`. The callback sees one row from `ducknng_auth_context()` and must return exactly one row with a Boolean `allow` column; optional columns are `status`, `reason`, `principal`, `claims_json`, and `cache_ttl_ms`. Keep it short and side-effect-light.
...
IP allowlisting is a lighter-weight gate than client certificates. `ducknng` includes a service-level IP/CIDR allowlist: for NNG transports it checks `NNG_OPT_REMADDR` at `ADD_PRE` and at dispatch; for HTTP/HTTPS it reads the connection remote address and returns HTTP `403` before RPC dispatch. A front door such as Caddy, nginx, or Envoy remains useful for public deployments to combine IP policy with rate limits, request logging, and WAF rules before forwarding to an internal NNG or HTTP service.
...
What is sealed and runnable in 0.1.0: NNG transports and socket patterns; first-class one-shot AIO across socket send/recv, ncurl HTTP, and unary RPC; framed RPC (`manifest`, opt-in `exec`, raw unary, query sessions); low-level HTTP routes with exact, prefix, or template matching, SQL response rows, request-context/body helpers, named request accessors, one-row response builders, route-local auth policies, static asset serving, background workers, and streaming response helpers (`ducknng_http_ndjson`, `ducknng_http_sse`); fast C admission (mTLS, exact peer-identity allowlists, IP/CIDR allowlists, per-service and per-principal resource limits); SQL authorizer callbacks; bounded per-service pipe-event monitor and active-pipe snapshot; body codec layer with built-in providers and user-registered codec hooks. See `function_catalog/functions.md` for the exact surface and `NEWS.md` for landed changes.
