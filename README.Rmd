...
- CSV/TSV/Parquet body parsing via DuckDB filesystem unstable API (in-memory buffer filesystem) to avoid the tempfile round-trip. Current implementations use a direct C parser for CSV/TSV and a tempfile-backed DuckDB reader for Parquet; standalone `ducknng_parse_csv`, `ducknng_parse_tsv`, `ducknng_parse_parquet` functions provide the DuckDB reader path explicitly.
- Full SQL-side decoding of session `fetch` Arrow batch BLOBs into a table-function path. The current scan-phase conversion uses a hand-rolled nanoarrow-to-DuckDB-vector switch statement. The planned path is bind/init-phase conversion using `duckdb_schema_from_arrow` / `duckdb_data_chunk_from_arrow` (unstable API); a dedicated decode-thread approach is noted in `docs/types.md` as a future alternative for multi-batch streaming results.
