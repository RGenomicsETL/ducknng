...
The stable async contract is raw-result-first. NNG/RPC aio helpers collect raw frames through `ducknng_aio_collect(...)`, HTTP aio helpers collect HTTP-shaped rows through `ducknng_ncurl_aio_collect(...)`, and callers explicitly decode frames or bodies afterward. URL-launched NNG request aio helpers also keep the initial dial in NNG's background retry path, so launch returns a handle even if the listener is not up yet and the eventual terminal result still reflects the ordinary request operation. Structured async wrappers may be added later as conveniences, but they remain additive; aio handles represent one pending operation, not a background job or streaming protocol.
