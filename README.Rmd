...
A MotherDuck-style public gateway is still a good fit for this layer, but it should use a separate backend DuckDB process or runtime boundary. That multi-process sketch lives in `docs/motherduck_style_demo.md` instead of this README because keeping it honest requires a second live `ducknng` backend, and the README examples are kept runnable in one rendered session.
