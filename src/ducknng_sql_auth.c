...
/* ducknng_request_subject(): the verified caller of the RPC request whose SQL
 * is running, as the dispatcher recorded it. It is read-only and captured at
 * bind time on the request thread; outside a request it returns no rows. */
typedef struct {
    idx_t row_count;
    char *peer_identity;
    char *principal;
...
static void ducknng_request_subject_bind(duckdb_bind_info info) {
    ducknng_sql_context *ctx = (ducknng_sql_context *)duckdb_bind_get_extra_info(info);
    const ducknng_execution_subject *subject;
    ...
    memset(bind, 0, sizeof(*bind));
    subject = ducknng_runtime_current_thread_execution_subject_get(ctx->rt);
    if (subject) {
        bind->row_count = 1;
        bind->peer_identity = subject->peer_identity && subject->peer_identity[0] ?
            ducknng_strdup(subject->peer_identity) : NULL;
        bind->principal = subject->principal && subject->principal[0] ?
            ducknng_strdup(subject->principal) : NULL;
        bind->subject = subject->subject && subject->subject[0] ?
            ducknng_strdup(subject->subject) : NULL;
        bind->claims_json = subject->claims_json && subject->claims_json[0] ?
            ducknng_strdup(subject->claims_json) : NULL;
    }
    type = duckdb_create_logical_type(DUCKDB_TYPE_VARCHAR);
    duckdb_bind_add_result_column(info, "peer_identity", type);
